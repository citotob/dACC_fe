...
function TableBootstrap() {
  ...
  // fetch api
  const getDataRoleTable = () => {
    setloading(true);
    API.getDataRoleTable(queryStatus, pageNumber, usersPerPage)
      .then((res) => {
        if (res.status === 200) {
          // console.log(
          //   "API SUCCESS : Fetch Data Role Table Data : ",
          //   res.data
          // );
    ...
    return (
      ...
      >
        <div className={`modal-body ${style.modalBody}`}>
          <h5 className={style.title}>Verifikasi Role</h5>
  ...
  const modalComponentReject = () => {
    return (
      ...
      >
        <div className={`modal-body ${style.modalBody}`}>
          <h5 className={style.title}>Tolak Role</h5>
          <div style={{ textAlign: "center" }}>
            <h1 className={style.name}>{selectedTableData?.name ?? ""}</h1>
            <p>{selectedTableData?.role?.name ?? ""}</p>
          </div>
      ...
      <Row>
        <Col className='col-12'>
          <div className={`${alertVerifyStatus}`}>
            <Alert color='success'>Role berhasil di verifikasi!</Alert>
          </div>
          <div className={`${alertRejectStatus}`}>
            <Alert color='success'>Role berhasil di reject!</Alert>
          </div>
          ...
          <Card>
            <CardBody>
              <Nav tabs className='nav-tabs-custom'>
                ...
                <NavItem>
                  <NavLink
                    ...
                    onClick={() => {
                      toggleCustom("2");
                      dispatch(changeBreadcrumbItem("Role Aktif"));
                      setqueryStatus("status=aktif");
                      setpageNumber(1);
                    }}
                  >
                    <span className='d-none d-sm-block'>Role Aktif</span>
                  </NavLink>
                </NavItem>
                <NavItem>
                  <NavLink
                    ...
                    onClick={() => {
                      toggleCustom("3");
                      dispatch(changeBreadcrumbItem("Role Ditolak"));
                      setqueryStatus("status=tolak");
                      setpageNumber(1);
                    }}
                  >
                    <span className='d-none d-sm-block'>Role Ditolak</span>
                  </NavLink>
                </NavItem>
              ...
              >
                ...
                >
                  <option value=''>Pilih</option>
                  <option value='role'>Tipe Role</option>
                </select>
