...
function TableBootstrap() {
  ...
  // untuk "selected" tag <option>
  const [selectStateProv, setSelectStateProv] = useState(true);
  const [selectStateKab, setSelectStateKab] = useState(true);
  const [selecteStateKec, setSelecteStateKec] = useState(true);
  const [selectStateDesa, setSelectStateDesa] = useState(true);
    ...
    API.postAddPenugasan(formData)
      .then((res) => {
        if (res.status === 200) {
          ...
          console.log("resssssssssss dari assign penugasan", res);
          console.log(
            "resssssssssss dari assign penugasan",
            res.data.values.kode
          );
          // ================= copy dari asli:
          let formAssign = new FormData();
          // Asign Penugasan to User
          formAssign.append("user", userid);
          formAssign.append("kode", res.data.values.kode);
          formAssign.append("assignto", dataInstansi);
          formAssign.append("ke", "1");
          const objectData = {
            user: userid,
            kode: res.data.values.kode,
            assignto: dataInstansi,
            ke: "1",
          };
          // const assign = handleAssignTugas(objectData);
          API.postAssignPenugasan(objectData)
            .then((res) => {
              console.log("ASSIGN TUGAS success : ", res);
              setrefreshTable(!refreshTable);
            })
            .catch((err) => {
              console.log("ASSIGN TUGAS FAILED : ", err);
            });
        }
      })

      .catch((err) => {
        console.log("gagal kirimkan tugas ", err?.response);
                    ...
                    <td className={`${style.aksiButtonsWrapper}`}>
                      {data?.status?.length === 1 ? (
                        <button
                          ...
                          onClick={() => {
                            tog_tugaskan();
                            setdataLokasiSurvey(data._id);
                            if (activeTab === "ai") {
                              setdataJenisSurvey("AI");
                            } else if (activeTab === "bts") {
                              setdataJenisSurvey("BTS");
                            }
                          }}
                          data-toggle='modal'
                          data-target='#myModal'
  ...
  // console.log("kabupaten", selectedKabKota);
            ...
            <CardBody>
              <Nav tabs className='nav-tabs-custom'>
                <NavItem>
                  ...
                  >
                    <span className={`d-none d-sm-block ${style.pageFont}`}>
                      Akses Internet
                    </span>
                  </NavLink>
                </NavItem>
                <NavItem>
                  ...
                  >
                    <span className='d-none d-sm-block'>BTS</span>
                  </NavLink>
                </NavItem>
