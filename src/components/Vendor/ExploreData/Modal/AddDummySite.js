...
export default function ModalDummySite(props) {
	...
	const handleAddDummy = () => {
		...
		} else if (!regLng.exec(props.long)) {
			alert("Longitude antara -180 s/d 180");
		} else if (!regLat.exec(props.lat)) {
			alert("Latitude antara -90 s/d 90");
		} else if (
			parseFloat(props.long) <= 86.160622 ||
			parseFloat(props.long) >= 149.441872
		) {
			alert("Longitude berada di luar jangkauan");
		} else if (
			parseFloat(props.lat) <= -17.545353 ||
			parseFloat(props.lat) >= 18.118934
		) {
