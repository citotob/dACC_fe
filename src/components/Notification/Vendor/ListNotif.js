...
const ListNotif = (props) => {
	...
	const handleClickNotif = (e, val, status) => {
		let Notif = val
			...
			: "";
		let batch_id = val.data.batch_id ? val.data.batch_id : "";
		let judul = val.data.judul ? encodeURIComponent(val.data.judul.trim()) : "";
		if (status !== "clicked") {
		...
		setTimeout(() => {
			if (Notif === "batchsent") {
				if (batch_id !== "" && judul !== "") {
					...
					localStorage.setItem("judul_batch_vendor", val?.data?.judul);
					history.push(`/vendor/sm/penawaran/${batch_id}/${judul}`);
				} else history.push("/vendor/site-matchmaking");
			}
			// else if (Notif === "batchreceived") {
