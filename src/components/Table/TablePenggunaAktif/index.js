...
// Create Data for Row
function createData(nama, username, tipe, email, no_telp, instansi, dokumen_penunjang, button_red) {
  return { nama, username, tipe, email, no_telp, instansi, dokumen_penunjang, button_red };
}
...
function TablePenggunaAktif(props) {
  // state
  ...
  // handle set data pdf and toggle modal
  ...
  async function fetchAPIPenggunaAktif(isSubscribed) {
    const getPenggunaAktif = await fetch(
      `${process.env.REACT_APP_BE_URL}/user/get/?status=verified`
    ).then((result) => result.json());

    setLoading(false);
    if (isSubscribed && getPenggunaAktif.success) {
      setTableStatus(true);
      const rowsData = [];
      for (let i of getPenggunaAktif.values) {
  ...
  useEffect(() => {
    ...
    fetchAPIPenggunaAktif(isSubscribed);
              ...
              <tbody>
                {items.length > 0 && filter != null ? (
                  items.slice(currentPage * pageItems, currentPage * pageItems + pageItems)
                    .map((row, index_row) => {
                      return (
                        <tr className={`${styles.border_laporan}`} key={index_row}>
                          <td
                            style={{
                              textAlign: 'left',
                              verticalAlign: 'middle',
...
export default TablePenggunaAktif;
