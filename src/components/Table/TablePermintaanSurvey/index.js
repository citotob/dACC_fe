            ...
            .map((row, index_row) => {
              return (
                <tr key={index_row} className={`${styles.border_laporan}`} style={{ fontSize: "15px" }}>
                  ...
                  {/* <td>{row.data[0].user.organization.name}</td> */}
                  <td>
                    {get(row, "data[0].user.organization.name", "-")}
                  </td>
                  {/* <td>{row.lokasi.desa !== undefined ? row.lokasi.desa.name : '-'}</td> */}
                  <td>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (Object.prototype.hasOwnProperty.call(row, 'flowbaru')) {
                          history.push('/app/admin/hasilsurvey/detail/'+row.kode)
                        } else {
                          //flow lama
                          // doc: set state content to hasil , send to hasilsurvey admin pages, switch to hasil survey detail
                          props.setContent(1);
...
function TablePermintaanSurvey(props) {
  ...
  return (
    <>
      <div className={`${styles.wrapperTable} pt-2 border-top`}>
        ...
        {dataResult.length === 0 ? ( // Tidak ada data
...
export default TablePermintaanSurvey;
