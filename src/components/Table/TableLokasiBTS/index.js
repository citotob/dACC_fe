...
// styling
import LokasiBTS from "./lokasibts.module.css";
// component
import PopupTugasBTS from "../../Popup/PopupTugasBTS";
...
const columns = [
  {
    ...
    textAlign: "left",
    width: "8%",
    className: `${LokasiBTS.content_report_table}`,
  },
  {
    id: "kabupatenkota",
  ...
  },
  {
    ...
    textAlign: "center",
    width: "4%",
    className: `${LokasiBTS.content_report_table} columnright`,
  },
  {
    id: "button_red",
    label: "",
...
function TableLokasiBTS(props) {
  ...
  const [modalTugasBTS, setModalTugasBTS] = useState(false);
  ...
  // toggle modal tugas
  let toggleTugasBTS = (i) => {
    setDataTugas(i);
    setModalTugasBTS(!modalTugasBTS);
  };
            ...
            if (!i.status[1]) {
              const buttonAction = (
                <div style={{ display: "inline" }}>
                  <Button
                    className={LokasiBTS.buttonTugas}
                    onClick={() => toggleTugasBTS(i)}
                  >
                    Tugaskan
                  </Button>
                </div>
  ...
  const handleAlert = (bool) => {
    ...
    props.handleAlert(bool, 'Penugasan BTS')
  ...
  return (
    <>
      ...
      <div className={LokasiBTS.wrapper_table}>
        <Row
          className={`${LokasiBTS.wrapper_filter} d-flex flex-column align-items-stretch border-top border-bottom`}
        >
            ...
            <CardBody>
              ...
              <Row className={"justify-content-center mt-4"}>
                <Button
                  size="sm"
                  color=""
                  className={LokasiBTS.btn_search}
                  onClick={(e) => handleSubmit(e)}>
                  Search
                </Button>
                <Button size="sm" color="" className={LokasiBTS.btn_reset} onClick={handleReset}>
                  Reset
                </Button>
              </Row>
            </CardBody>
          ...
          <Card className={"d-block d-md-none h-auto w-100 border-0 bg-light"}>
            <CardBody className={LokasiBTS.less_padding}>
              <h6>Filter</h6>
              <FilterBox
                handleFilter={handleTermProvinsi}
                handleReset={handleReset}
        ...
        {rows.length === 0 ? (
          ...
          <Table borderless responsive hover style={{ fontSize: "0.9rem" }}>
            <thead>
              <tr className={`${LokasiBTS.content_report_table} head`}>
                <th
                  style={{
                    textAlign: 'left',
                    verticalAlign: 'middle',
            ...
            <tbody>
              {items.length > 0 && filter != null ? (
                items.slice(currentPage * pageItems, currentPage * pageItems + pageItems)
                  .map((row, index_row) => {
                    return (
                      <tr key={index_row} className={`${LokasiBTS.border_laporan}`} >
                        <td
                          style={{
                            textAlign: 'left',
                            verticalAlign: 'middle',
        ...
        <Fragment>
          {items.length > 0 && filter != null ? (
            <div className={LokasiBTS.pagination}>
              <Pagination>
      ...
      <PopupTugasBTS
        ...
        modalTugasBTS={modalTugasBTS}
        toggleTugasBTS={() => toggleTugasBTS("")}
        data={dataTugas}
