...
const CollapseItem = (props) => {
  ...
  // const date = props.row.status[0].date;
  // const tgglHasil = moment(date, "YYYY-MM-DD hh:mm:ss.SSSSSS").format("DD/MM/YYYY");
...
function TableLogPenugasanSurveyor(props) {
  ...
  const [startDate, setStartDate] = useState(new Date()); //Tanggal SLA Penyelesaian
  const [endDate, setEndDate] = useState(new Date()); //Tanggal SLA Penyelesaian
  ...
  const fetchData = async (isSubscribed) => {
    ...
    const url = `${process.env.REACT_APP_BE_URL}/survey/getpenugasansurveyor/`;
...
export default TableLogPenugasanSurveyor;
