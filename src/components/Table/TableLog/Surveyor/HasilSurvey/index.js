  ...
  return (
    <>
      <tr key={props.index} className={`${styles.border_laporan}`} style={{ fontSize: "15px" }}>
        ...
        <td>
          <Button color="" className={styles.btn_icon} onClick={toggle}>
...
function TableLogHasilSurveySurveyor(props) {
  ...
  const fetchData = async () => {
    try {
      ...
      const url =
        props.tab === "AI"
          ? `${process.env.REACT_APP_BE_URL}/survey/getsurveyorlogai/`
          : `${process.env.REACT_APP_BE_URL}/survey/getsurveyorlogbts/`;
      ...
      let tempresarr = request.status === 200 ? response?.values ?? [] : [];
      console.log(response)
      setLoading(false);
      setDataStatus(['Submitted', 'Reviewed', 'Done', 'Finished'])

      if (props.tab === "AI") {
        setDataLog(tempresarr);
        setDataResult(tempresarr);
      } else {
        let fixlog = [];

        fixlog = tempresarr.map((arr) => {
          return {
        ...
        let request2 = await fetch(
          `${process.env.REACT_APP_BE_URL}/survey/getLogBtsNew/`,
          requestOption,
        );

        let response2 = request2.status === 200 && "json" in request ? await request2.json() : [];

        let tempresarr2 = request2.status === 200 ? response2?.values ?? [] : [];
        let fixlog2 = []

        fixlog2 = tempresarr2.map((arr) => {
          return {
            "kode": arr.data[0].kodeHasilSurvey,
            "status": arr.data[0].status
          }
        ...
        setDataLog(fixlog2.concat(fixlog));
        setDataResult(fixlog2.concat(fixlog));
      }
    } catch (error) {
      console.log(error)
    }
    ...
    <>
      <div className={`${styles.wrapperTable} pt-2 border-top`}>
        ...
        <Row className={`justify-content-center mb-3 mt-2`}>
          ...
          <Button size="sm" color="" className={styles.btn_search} onClick={handleReset}>
            Reset
          </Button>
        </Row>
...
export default TableLogHasilSurveySurveyor;
