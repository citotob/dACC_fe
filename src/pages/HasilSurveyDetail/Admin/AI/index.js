        ...
        <Col xs="5">
          <Row className={`${styles.row_data_collapse}`}>
            ...
            {Object.entries(data).map(([key, value], index) => {
              ...
              return (
                <>
                  <Button color="" key={index} onClick={() => toggleImage(source)} className={styles.btn_img}>
...
const ContentHasilSurvey = ({ tab, setContent, dataDetail, reload, handleAlert }) => {

  // modal confirm approve
  const [modalConfirmApprove, setModalConfirmApprove] = useState(false);
  // toggle modal confirm approve
  const toggleModalConfirmApprove = () => setModalConfirmApprove(!modalConfirmApprove);
  ...
  return (
    <>
      ...
      <PopupConfirmHasilSurvey
        kode={dataDetail.data[0].kodeHasilSurvey}
          ...
          <div className={`${styles.wrapper_title} d-flex justify-content-between`}>
            ...
            <div>
              <Button className={`ml-5 mr-15 ${styles.btnDownloadPDF}`}>
                {isReady ? <DownloadPDF dataDetail={dataDetail} jenisSurvey={jenis_survey} issue={false} /> : ""}
                {/* PDF */}
              </Button>
            </div>
          </div>
                    ...
                    <div className={styles.row_detail}>
                      <p>
                        ...
                        {get(dataDetail, 'data[0].status[0].tanggal_pembuatan')
                          ...
                          : '-'}
                        {/* Below is date for latest status */}
                        {/* {dataDetail.data[0].status.slice(-1).pop().tanggal_pembuatan
                          ? moment(
                            dataDetail.data[0].status.slice(-1).pop().tanggal_pembuatan
                          ).format("DD/MM/YYYY")
                          : "-"} */}
...
// Change status Penugasan to reviewed upon open
const updateStatusPenugasan = (dataDetail) => {
  return fetch(`${process.env.REACT_APP_BE_URL}/survey/changestatuspenugasan/`, {
    ...
    method: "POST",
    body: JSON.stringify({ kode: dataDetail.data[0].kodeHasilSurvey, status: "reviewed" }),
  })
...
const HasilSurveyDetailAdminAI = (props) => {
  // Change status Penugasan to reviewed upon open
  updateStatusPenugasan(props.data);
...
export default HasilSurveyDetailAdminAI;
