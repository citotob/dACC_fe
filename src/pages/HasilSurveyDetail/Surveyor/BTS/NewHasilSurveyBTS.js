...
import { allpage } from "./JSONLabel/allpage"
...
const NewHasilSurveyBTS = (props) => {
	...
	async function fetchwa(kode_survei) {
		if (!alreadyFetch) {
			...
			try {
                ...
                // check status review
                response2.values[0].data.map((data, index) => {
                    if (data.status.slice(-1).pop().status == "Submitted") {
                        let updatestatus = axiosPost(`${process.env.REACT_APP_BE_URL}/survey/changestatusbtsnew/`, {
                            "kode": kode_survei
                        })
                    }
                })
	...
	async function handleSetujuiSurvey() {
		try {
			...
			setLoadingFetch(true)
			const data = await axiosPost(`${process.env.REACT_APP_BE_URL}/survey/approvesurveibts/`, {
			...
			if (data.data.success) {
				...
				setTimeout(() => {
					setOpenSukses(false)
					history.push("/app/adminsurveyor/hasilsurvey")
				}, 2000);
			} else {
				setMessage(data.data.message)
				setOpenFailed(true)
	...
	async function handleTolak() {
		try {
			setModalTolak(false)
			setLoadingFetch(true)
			const data = await axiosPost(`${process.env.REACT_APP_BE_URL}/survey/tolaksurveibts/`, {
				kode_survei: kode_survei,
				alasan: alasan,
				userfrom: userid,
			})
	...
	return loading ? LoadingComp() : errorfetch ? ErrorFetchComp() : (
		<div>
			<ModalFlag
																						...
																						data[1].map((datum, i) => {
																							...
																							return (
																								<>
																									<tr>
																										<td style={{ width: "5%" }}>
																											<button onClick={() => { handleFlag(data[3], i, index) }} style={{ padding: "0px" }} className="btn">
																												{determineFlag(data[3], i, index)}
																											</button>
																										</td>
																										<td>
																								...
																								data[1].map((datum, i) => {
																									...
																									return (
																										<>
																											<tr>
																												<td style={{ width: "5%" }}>
																													<button onClick={() => { handleFlag("photocapturegnettrack5km", i, index) }} style={{ padding: "0px" }} className="btn">
...
export default NewHasilSurveyBTS
